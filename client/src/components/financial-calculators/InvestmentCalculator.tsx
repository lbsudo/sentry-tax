import { useState } from "react";

export default function InvestmentsCalculator() {
    const [isModuleOpen, setIsModuleOpen] = useState(false);

    return (
        <>
            {/* MAIN CARD */}
            <div
                className="
          max-w-4xl mx-auto
          bg-white/5 backdrop-blur-xl
          border border-white/10
          rounded-2xl p-6 sm:p-8
          shadow-[0_8px_30px_rgba(0,0,0,0.4)]
          transition-all duration-500
          hover:shadow-[0_12px_40px_rgba(0,0,0,0.55)]
        "
            >
                {/* Title */}
                <h2 className="text-2xl sm:text-3xl font-semibold text-white mb-6">
                    Investments Calculator
                </h2>

                <div className="flex flex-col lg:flex-row items-start gap-8">

                    {/* BUTTON */}
                    <button
                        onClick={() => setIsModuleOpen(true)}
                        className="
              px-6 py-3 rounded-lg text-lg font-medium
              border border-[#27651c]/60
              text-white
              hover:bg-[#27651c] hover:text-white
                            shadow-[0_0_15px_rgba(255,255,255,0.4)]
              hover:shadow-[0_0_20px_rgba(39,101,28,0.7)]
              transition-all duration-300
              w-full lg:w-auto
            "
                    >
                        Run Investments Calculator
                    </button>

                    {/* List of tools */}
                    <div className="text-gray-300 text-lg">
                        <ul className="space-y-2 list-disc ml-5">
                            <li>What is the Yield to Maturity?</li>
                            <li>What Selling Price Provides Desired Return?</li>
                        </ul>
                    </div>

                </div>
            </div>

            {/* MODAL */}
            {isModuleOpen && (
                <div
                    className="
            fixed inset-0 z-50
            bg-black/70 backdrop-blur-sm
            flex justify-center items-center p-4
            animate-[fadeIn_0.3s_ease-out]
          "
                    onClick={() => setIsModuleOpen(false)}
                >
                    {/* MODAL CONTENT */}
                    <div
                        className="
              bg-[#0d0d0d] text-white rounded-xl
              border border-white/10
              shadow-[0_0_40px_rgba(0,0,0,0.6)]
              w-full max-w-4xl h-[85vh]
              flex flex-col overflow-hidden
              transform transition-all duration-300
              animate-[scaleFadeIn_0.25s_ease-out]
            "
                        onClick={(e) => e.stopPropagation()}
                    >
                        {/* Header */}
                        <div
                            className="
                bg-[#27651c]
                px-5 py-4 text-xl font-semibold
                flex justify-between items-center
                shadow-[0_4px_20px_rgba(39,101,28,0.5)]
              "
                        >
                            <span>Investments Calculator</span>
                            <button
                                onClick={() => setIsModuleOpen(false)}
                                className="text-white text-3xl leading-none hover:opacity-70 transition"
                            >
                                ×
                            </button>
                        </div>

                        {/* Embedded Calculator */}
                        <iframe
                            src="https://www.calculator.net/investment-calculator.html"
                            className="w-full h-full bg-white"
                            title="Investments Calculator"
                        />
                    </div>
                </div>
            )}
        </>
    );
}
